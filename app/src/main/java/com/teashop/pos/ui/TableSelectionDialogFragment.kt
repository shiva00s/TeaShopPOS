...
class TableSelectionDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding.btnStanding.setOnClickListener {
            viewModel.setServiceType("STANDING")
            dismiss()
        }
        binding.btnParcel.setOnClickListener {
            viewModel.setServiceType("PARCEL")
            dismiss()
        }

        val tables = (1..10).map { "T$it" }
        val adapter = TableGridAdapter(tables) { tableId ->
            viewModel.setServiceType("TABLE", tableId)
            dismiss()
        }

        binding.rvTables.layoutManager = GridLayoutManager(context, 3) // Adjust span count as needed
